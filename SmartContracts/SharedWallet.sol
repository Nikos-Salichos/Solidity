...
contract Allowance is Ownable{
    ...
    event AllowanceChanged(address indexed forAddress, address indexed fromAddress, uint256 oldAmount, uint256 newAmount);

    uint256 public amountReceived;
    mapping(address => uint256) public allowance;

    modifier onwnerOrAllowed(uint256 amount){
        require(owner() ==  msg.sender || allowance[msg.sender] >= amount , "You are not allowed to withdraw");
        _;
    }
    ...
    function reduceAllowance(address who, uint256 amount) internal{
    ...
    function renounceOwnership() override public view onlyOwner{
        revert("You cannot renounce ownership here");
    }
